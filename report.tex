...
\vspace{1em}
\noindent
Criminal prosecution in Sindh is governed by a dense and fragmented body of primary legislation --- the Sindh Criminal Court Rules 2012, the Sindh Criminal Prosecution Service Act 2009, and a sequence of amendment Acts spanning 2011, 2014, 2015, and 2025 --- much of which is scattered across official gazettes, High Court publications, and unindexed PDFs. Practising prosecutors, defence advocates, and law clerks routinely need to answer precise procedural questions (``what are the qualifications for appointment of the Prosecutor General?'', ``what is the procedure for recording a confession?'', ``what rules govern the production of digital video evidence?'') but must currently locate the answer by manually searching multiple documents, cross-referencing amendments, and reconciling consolidated versions. This is slow, error-prone, and poorly suited to the tight timelines of active litigation. We chose Sindh Criminal Prosecution Law as our project domain precisely because it exhibits this combination --- high procedural specificity, heavy reliance on exact section numbers, and no publicly available purpose-built search tool --- which makes it an ideal target for a retrieval-augmented generation system. A well-grounded RAG system offers lawyers a concrete productivity gain: it returns the governing provision verbatim, cites the exact rule or section, and refuses to fabricate content when the source is silent, yielding answers a practitioner can confidently carry into court.
